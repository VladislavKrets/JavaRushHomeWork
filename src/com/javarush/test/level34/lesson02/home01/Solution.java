...
public class Solution {
    ...
    public void recursion(final String expression, int countOperation) {
        ...
        if (p3 != -1) {
            ...
            switch (s[p3]) {
                case '^':
                    customPrint(df, Math.pow(Double.parseDouble(df.format(Double.parseDouble(part1[0]))), Double.parseDouble(df.format(Double.parseDouble(part2[0])))), countOperation, currentCountOperation);
                    return;
                case '*':
                    customPrint(df, Double.parseDouble(df.format(Double.parseDouble(part1[0]))) * Double.parseDouble(df.format(Double.parseDouble(part2[0]))), countOperation, currentCountOperation);
                    return;
                case '/':
                    customPrint(df, Double.parseDouble(df.format(Double.parseDouble(part1[0]))) / Double.parseDouble(df.format(Double.parseDouble(part2[0]))), countOperation, currentCountOperation);
                    return;
